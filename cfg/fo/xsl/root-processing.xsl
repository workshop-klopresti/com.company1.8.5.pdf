...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="retain-bookmap-order"
    select="*[contains(@class, ' bookmap/bookmap ')] and $bookmap-order eq 'retain'"/>
  ...
  <xsl:variable name="relatedTopicrefs"
    select="//*[contains(@class, ' map/reltable ')]//*[contains(@class, ' map/topicref ')]"/>
  ...
  <xsl:variable name="productRelease"
    select="(/*/opentopic:map//*[contains(@class,' topic/vrm ')]/@release)[1] "/>-->
  ...
  <xsl:variable name="itemNumber"
    select="//*[contains(@class, ' bookmap/bookid ')]//*[contains(@class, ' bookmap/booknumber ')]"/>
  ...
  <xsl:template match="*[contains(@class, ' map/topicref ')]" mode="topicref-validation">
    ...
    <xsl:if test="@href and @id">
      ...
      <xsl:if
        test="not(//*[contains(@class, ' topic/topic ')][@id = $searchId]) and not($searchId = '')">
        <xsl:call-template name="output-message">
    ...
    <xsl:choose>
      <xsl:when
        ...
        <xsl:for-each
          ...
          <xsl:value-of>
            <xsl:apply-templates select="*[contains(@class, ' xnal-d/firstname ')]/node()"
              mode="dita-ot:text-only"/>
            <xsl:text> </xsl:text>
            <xsl:apply-templates select="*[contains(@class, ' xnal-d/lastname ')]/node()"
              mode="dita-ot:text-only"/>
          </xsl:value-of>
        </xsl:for-each>
      </xsl:when>
      ...
      <!-- This statement places <author> value in PDF properties.      -->
      <xsl:when
  ...
  <xsl:template match="/" mode="dita-ot:keywords-metadata" as="xs:string*">
    <xsl:variable name="keywords"
      select="$map/*[contains(@class, ' bookmap/bookmeta ')]/*[contains(@class, ' topic/keywords ')]/*[contains(@class, 'topic/keyword ')]"/>
  ...
  <xsl:template match="/" mode="dita-ot:subject-metadata" as="xs:string?">
    <xsl:choose>
      <xsl:when
        test="exists($map/*[contains(@class, ' bookmap/bookmeta ')]/*[contains(@class, ' map/shortdesc ')])">
        <xsl:value-of>
          <xsl:apply-templates
      ...
      <xsl:when test="exists($map/*[contains(@class, ' topic/shortdesc ')])">
        <xsl:value-of>
          <xsl:apply-templates select="$map/*[contains(@class, ' topic/shortdesc ')]"
            mode="dita-ot:text-only"/>
        </xsl:value-of>
      </xsl:when>
    </xsl:choose>

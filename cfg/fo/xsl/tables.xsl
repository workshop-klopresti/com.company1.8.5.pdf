<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    exclude-result-prefixes="exsl exslf opentopic-func xs dita2xslfo" version="2.0">
    ...
    <!--  KLL 2016-10-07 this is the default 2-column table format for defintion lists -->    
    ...
    <xsl:template
        match="*[contains(@class, ' topic/sthead ')] | *[contains(@class, ' topic/strow ')]"
        ...
        <xsl:variable name="newmaxcount" as="xs:integer">
            <xsl:variable name="row-cell-count"
                select="count(*[contains(@class, ' topic/stentry ')])"/>
            <xsl:choose>
                <xsl:when test="$row-cell-count > $maxcount">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:apply-templates
                    select="following-sibling::*[contains(@class, ' topic/strow ')][1]"
                    mode="count-max-simpletable-cells">
    ...
    <xsl:template name="xcalcColumnWidth">
        ...
        <xsl:variable name="width-units">
            <xsl:choose>
                <xsl:when test="contains($theColwidth, '*')">
                    <xsl:value-of select="normalize-space(substring-after($theColwidth, '*'))"/>
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:variable name="width"
            select="
                normalize-space(translate($width-units,
    ...
    <xsl:template name="univAttrs">
        <xsl:apply-templates
            select="@platform | @product | @audience | @otherprops | @importance | @rev | @status"/>
    </xsl:template>
    ...
    <xsl:template match="*[contains(@class, ' topic/tgroup ')]" name="tgroup">
        ...
        <xsl:variable name="table">
            <fo:table xsl:use-attribute-sets="table.tgroup" table-omit-header-at-break="false"
            ...
            <!--DITA for Print-->
            <!-- <xsl:apply-templates select="preceding-sibling::*[contains(@class, ' topic/title ')]"
                mode="titleBelow"/>-->
        </xsl:variable>
    ...
    <xsl:template name="processEntryContent">
        ...
        <xsl:variable name="char">
            <xsl:choose>
                ...
                <xsl:when
                    test="ancestor::*[contains(@class, ' topic/tgroup ')][1]/*[contains(@class, ' topic/colspec ')][position() = number($entryNumber)]/@char">
                    <xsl:value-of
                        select="ancestor::*[contains(@class, ' topic/tgroup ')][1]/*[contains(@class, ' topic/colspec ')][position() = $entryNumber]/@char"
                    />
                </xsl:when>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="charoff">
            <xsl:choose>
                ...
                <xsl:when
                    test="ancestor::*[contains(@class, ' topic/tgroup ')][1]/*[contains(@class, ' topic/colspec ')][position() = number($entryNumber)]/@charoff">
                    <xsl:value-of
                        select="ancestor::*[contains(@class, ' topic/tgroup ')][1]/*[contains(@class, ' topic/colspec ')][position() = $entryNumber]/@charoff"
                    />
                </xsl:when>
    ...
    <xsl:template name="processCharAlignment">
        ...
        <xsl:choose>
            <xsl:when test="not(descendant::*)">
                ...
                <fo:list-block start-indent="0in" provisional-label-separation="0pt"
    ...
    <xsl:template name="countEntryNumber">
        <xsl:choose>
            <xsl:when test="@colname">
                ...
                <xsl:if
                    test="ancestor::*[contains(@class, ' topic/tgroup ')][1]/*[contains(@class, ' topic/colspec ')][@colname = $colname]">
                    <xsl:number
                        select="ancestor::*[contains(@class, ' topic/tgroup ')][1]/*[contains(@class, ' topic/colspec ')][@colname = $colname]"
                    />
                </xsl:if>
            </xsl:when>
...
-->
    ...
    <xsl:template name="getEntryNumber">
        ...
        <xsl:choose>
            ...
            <xsl:when
                ...
                <xsl:for-each
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:value-of
                                select="count(preceding-sibling::*[contains(@class, ' topic/colspec ')]) + 1"
                            />
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:for-each>
            ...
            <xsl:when
                test="not($optionalName = '') and ancestor::*[contains(@class, ' topic/tgroup ')][1]/*[contains(@class, ' topic/colspec ')][@colname = $optionalName]">
                <xsl:for-each
                    select="ancestor::*[contains(@class, ' topic/tgroup ')][1]/*[contains(@class, ' topic/colspec ')][@colname = $optionalName]">
                    <xsl:choose>
                        <xsl:when test="@colnum">
                            <xsl:value-of select="@colnum"/>
                        </xsl:when>
            ...
            <xsl:when
                test="not(string(number(translate($colname, '+-0123456789.abcdefghijklmnopqrstuvwxyz', '0123456789'))) = 'NaN')">
                <xsl:value-of
                    select="number(translate($colname, '0123456789.abcdefghijklmnopqrstuvwxyz', '0123456789'))"
                />
            </xsl:when>
    ...
    <xsl:template name="applySpansAttrs">
        ...
        <xsl:if test="(@nameend) and (@namest)">
            ...
            <xsl:if
                test="($startNum > '-1') and ($endNum > '-1') and ((number($endNum) - number($startNum)) > 0)">
    ...
    <xsl:template name="generateTableEntryBorder">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="../parent::node()[contains(@class, ' topic/tbody ')]">
                            ...
                            <xsl:variable name="prevEntryRowsep">
                                <xsl:for-each
                                    select="../preceding-sibling::*[contains(@class, ' topic/row ')]/*[contains(@class, ' topic/entry ')][$entryNum]">
                                    <xsl:call-template name="getTableRowsep"/>
                                </xsl:for-each>
                            </xsl:variable>
                            <xsl:choose>
        ...
        <xsl:if
            test="number($rowsep) = 1 and (../parent::node()[contains(@class, ' topic/thead ')])">
        ...
        <xsl:if
            test="number($colsep) = 1 and following-sibling::*[contains(@class, ' topic/entry ')]">
            <xsl:call-template name="processAttrSetReflection">
            ...
            <xsl:when test="contains($theColumnWidthes, ' ')">
                    ...
                    <xsl:attribute name="column-width">
                        <xsl:call-template name="xcalcColumnWidth">
                            <xsl:with-param name="theColwidth"
                                select="substring-before($theColumnWidthes, ' ')"/>
                        </xsl:call-template>
                    </xsl:attribute>
                </fo:table-column>
                ...
                <xsl:call-template name="createSimpleTableColumns">
                    <xsl:with-param name="theColumnWidthes"
                        select="substring-after($theColumnWidthes, ' ')"/>
                </xsl:call-template>
    ...
    <xsl:template match="*[contains(@class, ' topic/simpletable ')]"
        mode="dita2xslfo:simpletable-heading">
        <xsl:param name="number-cells">
            <xsl:apply-templates select="*[1]" mode="count-max-simpletable-cells"/>
        </xsl:param>
        ...
        <fo:table-header xsl:use-attribute-sets="sthead">
            ...
            <fo:table-row xsl:use-attribute-sets="sthead__row">
                ...
                <xsl:if test="$row-cell-count &lt; $number-cells">
                    <xsl:apply-templates select="." mode="fillInMissingSimpletableCells">
                        <xsl:with-param name="fill-in-count"
                            select="$number-cells - $row-cell-count"/>
                    </xsl:apply-templates>
                </xsl:if>
            </fo:table-row>
        </fo:table-header>
    ...
    <xsl:template
        match="*[contains(@class, ' topic/sthead ')]/*[contains(@class, ' topic/stentry ')]">
    ...
    <xsl:template name="generateSimpleTableHorizontalBorders">
        ...
        <xsl:choose>
            <xsl:when
                test="($frame = 'all') or ($frame = 'topbot') or ($frame = 'sides') or not($frame)">
                <xsl:call-template name="processAttrSetReflection">
                    <xsl:with-param name="attrSet" select="'__tableframe__bottom'"/>
                    <xsl:with-param name="path" select="$tableAttrs"/>
                </xsl:call-template>

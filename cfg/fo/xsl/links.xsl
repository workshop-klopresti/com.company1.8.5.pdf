...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	exclude-result-prefixes="opentopic-mapmerge opentopic-func exslf exsl" version="2.0">
	...
	<xsl:template
		match="*[contains(@class, ' topic/xref ') or contains(@class, ' topic/link ')]/*[contains(@class, ' topic/desc ')]"
		priority="1"/>
 ...
 <xsl:value-of select="count(preceding::*[contains(@class, ' topic/fig ')][child::*[contains(@class, ' topic/title ')]]) + 1"/>
	...
	<xsl:template match="*[contains(@class, ' topic/table ')][*[contains(@class, ' topic/title ')]]"
		...
		<xsl:call-template name="insertVariable">
			...
			<xsl:with-param name="theParameters">
				<number>
					<xsl:value-of
						select="count(preceding::*[contains(@class, ' topic/table ')][child::*[contains(@class, ' topic/title ')]]) + 1"
					/>
				</number>
				<title>
					<xsl:apply-templates select="*[contains(@class, ' topic/title ')]"
	...
	<xsl:template match="*[contains(@class, ' topic/xref ')][@type = 'fn']" priority="2">
		...
		<xsl:variable name="footnote-target"
			select="//*[contains(@class, ' topic/fn ')][@id = substring-after($href-fragment, '/')][ancestor::*[contains(@class, ' topic/topic ')][1]/@id = substring-before($href-fragment, '/')]"/>
		...
		<xsl:if test="normalize-space($includeRelatedLinkRoles)">
			...
			<xsl:if test="normalize-space($linkTextContent) != ''">
				<fo:block xsl:use-attribute-sets="related-links">
					...
					<!--DITA for Print-->
					<!-- KLL  25 August 2015 added padding to match topic spacing             	-->
					<fo:block-container width="3in" padding-top="6pt">
						<fo:block keep-with-next.within-page="always" xsl:use-attribute-sets="related-links.title" >
														<xsl:call-template name="insertVariable">
	...
	<xsl:template match="*[contains(@class, ' topic/link ')]">
		...
		<xsl:choose>
			<xsl:when
				test="
					(@role and not(contains($includeRelatedLinkRoles, concat(' ', @role, ' ')))) or
...
-->
			...
			<!--KLL 11 August 2015 Disabled shortdesc from appearing in Related Links  -->
	...
	<xsl:template name="insertPageNumberCitation">
		...
		<xsl:param name="element"/>
		<xsl:choose>
		<!-- KLL  29 February 2016 for links to steps in the same task do not output page number      	-->
			<xsl:when test="@type = 'step'">
...
-->
		...
		<xsl:choose>
			...
			<xsl:when
				test="($selfCollectionType = 'unordered') or ($selfCollectionType = 'choice') or ($selfCollectionType = 'family')">
				<xsl:call-template name="linkToChilds">
		...
		<xsl:choose>
			<xsl:when
				test="($parentCollectionType = 'none') or ($parentCollectionType = 'unordered') or ($parentCollectionType = 'choice')">
			...
			<xsl:when test="$parentCollectionType = 'family'">
				...
				<!-- Creating relationships to the concepts -->
				<xsl:variable name="siblingConcepts"
					select="preceding-sibling::*[contains(@class, ' concept/concept ')] | following-sibling::*[contains(@class, ' concept/concept ')]"/>
				...
				<!-- Creating relationships to the tasks -->
				<xsl:variable name="siblingTasks"
					select="preceding-sibling::*[contains(@class, ' task/task ')] | following-sibling::*[contains(@class, ' task/task ')]"/>
				<xsl:call-template name="createMapLinks">
				...
				<!-- Creating relationships to the references -->
				<xsl:variable name="siblingReferences"
					select="preceding-sibling::*[contains(@class, ' reference/reference ')] | following-sibling::*[contains(@class, ' reference/reference ')]"/>
				<xsl:call-template name="createMapLinks">
				...
				<!-- Creating relationships to the topics -->
				<xsl:variable name="siblingTopics"
					select="preceding-sibling::*[contains(@class, ' topic/topic ') and not(contains(@class, ' concept/concept ') or contains(@class, ' task/task ') or contains(@class, ' reference/reference '))] | following-sibling::*[contains(@class, ' topic/topic ') and not(contains(@class, ' concept/concept ') or contains(@class, ' task/task ') or contains(@class, ' reference/reference '))]"/>
				<xsl:call-template name="createMapLinks">
	...
	<xsl:template name="linkToNext">
		...
		<xsl:call-template name="createMapLinks">
			<xsl:with-param name="nodeSet"
				select="following-sibling::*[contains(@class, ' topic/topic ')][1]"/>
			<xsl:with-param name="title" select="$linksTitle"/>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template name="linkToPrevious">
		...
		<xsl:call-template name="createMapLinks">
			<xsl:with-param name="nodeSet"
				select="preceding-sibling::*[contains(@class, ' topic/topic ')][1]"/>
			<xsl:with-param name="title" select="$linksTitle"/>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template name="linkToChilds">
		...
		<xsl:call-template name="createMapLinks">
			<xsl:with-param name="nodeSet"
				select="*[contains(@class, ' topic/topic ')] | *[contains(@class, ' topic/dita ')]/*[contains(@class, ' topic/topic ')]"/>
			<xsl:with-param name="title" select="$linksTitle"/>
		...
		<xsl:if test="$relatedTopicrefs/@id = $id">
			<xsl:variable name="resultLinks">
				<xsl:for-each select="$relatedTopicrefs[@id = $id]">
					<xsl:choose>
						<xsl:when
							test="ancestor::*[contains(@class, ' map/reltable ')]/*[contains(@class, ' map/relheader ')]">
							<xsl:variable name="topicTypeCellSpec"
								select="ancestor::*[contains(@class, ' map/reltable ')]/*[contains(@class, ' map/relheader ')]/*[contains(@class, ' map/relcolspec ')][@type = $linkType]"/>
							<xsl:if test="$topicTypeCellSpec">
								<xsl:variable name="currPosition"
									select="count(ancestor::*[contains(@class, ' map/relcell ')][1]/preceding-sibling::*) + 1"/>
								...
								<xsl:if test="not($currPosition = $position)">
									<xsl:for-each
										select="ancestor::*[contains(@class, ' map/relrow ')]/*[contains(@class, ' map/relcell ')][position() = $position]//*[contains(@class, ' map/topicref ')]">
										<xsl:variable name="relatedTopic"
											select="key('key_anchor', @id)[1]"/>
										<fo:block xsl:use-attribute-sets="related-links__content">
											<fo:basic-link internal-destination="{@id}"
												xsl:use-attribute-sets="xref">
												<xsl:apply-templates
												select="$relatedTopic/*[contains(@class, ' topic/title ')]"
												mode="insert-text"/>
											</fo:basic-link>
										</fo:block>
									</xsl:for-each>
						...
						<xsl:otherwise>
							<xsl:for-each
								select="ancestor::*[contains(@class, ' map/relcell ')][1]/preceding-sibling::*//*[contains(@class, ' map/topicref ')] | ancestor::*[contains(@class, ' map/relcell ')][1]/following-sibling::*//*[contains(@class, ' map/topicref ')]">
								<xsl:variable name="relatedTopic" select="key('key_anchor', @id)[1]"/>
								<xsl:choose>
									<xsl:when test="$linkType = 'topic'">
										<xsl:if
											test="contains($relatedTopic/@class, ' topic/topic ') and not(contains($relatedTopic/@class, ' concept/concept ') or contains($relatedTopic/@class, ' task/task ') or contains($relatedTopic/@class, ' reference/reference '))">
											<fo:block
												xsl:use-attribute-sets="related-links__content">
												<fo:basic-link internal-destination="{@id}"
												xsl:use-attribute-sets="xref">
												<xsl:apply-templates
												select="$relatedTopic/*[contains(@class, ' topic/title ')]"
									...
									<xsl:when test="$linkType = 'concept'">
										<xsl:if
											test="contains($relatedTopic/@class, ' concept/concept ')">
											<fo:block
												xsl:use-attribute-sets="related-links__content">
												<fo:basic-link internal-destination="{@id}"
												xsl:use-attribute-sets="xref">
									...
									<xsl:when test="$linkType = 'reference'">
										<xsl:if
											test="contains($relatedTopic/@class, ' reference/reference ')">
											<fo:block
												xsl:use-attribute-sets="related-links__content">
												<fo:basic-link internal-destination="{@id}"
												xsl:use-attribute-sets="xref">

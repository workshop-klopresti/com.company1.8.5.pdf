...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*[contains(@class, ' topic/topic ')]">
        ...
        <!-- TOMV DEBUG INFO: -->
        <!--
        <xsl:message>PROCESSSING TOPIC: <xsl:value-of select="*[contains(@class,' topic/title ' )]" /></xsl:message>
        <xsl:message>  TOPIC TYPE : <xsl:value-of select="$topicType" /></xsl:message>
        <xsl:message>   FUll @CLASS:<xsl:value-of select="@class"/></xsl:message>
        -->
        <xsl:choose>
            ...
            <xsl:when test="$topicType = 'topicSimple'">
                ...
                <xsl:choose>
                    <xsl:when
                        test="not(ancestor::*[contains(@class, ' topic/topic ')]) and not(ancestor::ot-placeholder:glossarylist)">
        ...
        <fo:block>
            ...
            <xsl:choose>
                    ...
                    <fo:block xsl:use-attribute-sets="__chapter__frontmatter__name__container">
                        <xsl:call-template name="insertVariable">
                            ...
                            <xsl:with-param name="theParameters">
                                ...
                                <!--  KLL 2016-06-29 Put part title on separate line below part number-->
                                <number>
                                    <fo:inline>
                ...
                <xsl:when test="$type = 'notices'">
                    <!-- TOMV (2014-11-14) For Notices section, supress the title  -->
                    <!-- 
                    <fo:block xsl:use-attribute-sets="__chapter__frontmatter__name__container">
                        <xsl:call-template name="insertVariable">
    ...
    <!-- MiniTOC in a table -->
    <!-- attributes for MiniToc are in toc-attr.xsl -->
        ...
        <fo:block xsl:use-attribute-sets="fig.title">
            ...
            <xsl:call-template name="insertVariable">
                ...
                <xsl:with-param name="theParameters">
                    ...
                    <!-- Get all the figures (with titles) in the current chapter -->
                    <xsl:variable name="chap_app_topic" select="key('id', $chap_app_id)"/>

                    <!-- List all the figures in the chapter with titles. -->
                    <xsl:variable name="chap_app_figs"
                        select="$chap_app_topic/descendant::*[contains(@class, ' topic/fig ') and child::*[contains(@class, ' topic/title ')]]"/>

                    <!-- Find the position of the figure of interest in the list. -->
                    <xsl:variable name="fig_num">
                        <xsl:for-each select="$chap_app_figs">
                            <xsl:if test="generate-id(.) = $this_fig_id">
                                <xsl:value-of select="position()"/>
                            </xsl:if>
                        </xsl:for-each>
                    </xsl:variable>
                    <!--                    <xsl:message>fig_num is <xsl:value-of select="$fig_num"/>.</xsl:message>
                    <!-\- Just not sure why this doesn't work...have to think about it for a bit.-\->    
                    <xsl:message>Provisional figure number is: <xsl:value-of 
                        select="count($chap_app_figs[generate-id(.) = $this_fig_id]/preceding-sibling::*) + 1"/>.</xsl:message>
-->
                    <number>
                        <xsl:call-template name="getChapterPrefix"/>
                        <xsl:text>-</xsl:text>
    ...
    <xsl:template match="*[contains(@class, ' topic/tm ')]" priority="2">
        ...
        <!-- [SP] 2017-03-10 sfb: When found in title or mainbooktitle doesn't count as first usage. -->
        <!-- Also added missing "contains()" on class predicate. -->
        <xsl:variable name="isNotFirst" as="xs:boolean"
            select="boolean(preceding::*[not(ancestor::opentopic:map)][contains(@class, ' topic/tm ')][not(ancestor::*[contains(@class, ' topic/title ')])][normalize-space(.) = ($tmText)])"/>
        <xsl:variable name="isNotFirstMainbooktitle" as="xs:boolean"
            select="boolean(preceding::*[contains(@class, ' topic/tm ')][ancestor::*[contains(@class, ' bookmap/mainbooktitle ')]][normalize-space(.) = ($tmText)])"/>
        <xsl:choose>
            <!-- [SP] 2017-03-13 sfb: When found in mainbooktitle, give it a TM. -->
            <xsl:when test="ancestor::*[contains(@class, ' bookmap/mainbooktitle ')]">
                <xsl:next-match/>
            </xsl:when>
            <!-- [SP] 2017-03-10 sfb: When found in a title, don't bother with TM. -->
            <xsl:when test="ancestor::*[contains(@class, ' topic/title ')]">
                <xsl:value-of select="$tmText"/>
            </xsl:when>
            <!-- output without the tm sign when it is not the first, non-title use -->
    ...
    <xsl:template match="*[contains(@class, ' topic/term ')]" name="topic.term">
        ...
        <xsl:param name="contents" as="node()*">
            ...
            <xsl:choose>
                <xsl:when
                    ...
                    <xsl:apply-templates
                        select="$target/*[contains(@class, ' topic/title ')]/node()"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates/>
                </xsl:otherwise>
        ...
        <xsl:choose>
            <xsl:when
                test="
                    $keys and @href and not($topicref/ancestor-or-self::*[@linking][1]/@linking = ('none',
                    'sourceonly'))">
...
-->
    ...
    <xsl:template name="check_reltable">
        ...
        <xsl:if test="count($required) &gt; 0">
            ...
            <fo:block xsl:use-attribute-sets="prereq" keep-with-next="always">
                <fo:block xsl:use-attribute-sets="section.title" keep-with-next.within-page="always">
                    <fo:inline>
                        <xsl:call-template name="insertVariable">
    ...
    <xsl:template
        match="
            *[contains(@class, ' topic/section ')][@spectitle != ''
    ...
    <xsl:template match="*[contains(@class, ' topic/example ')]">
        <fo:block xsl:use-attribute-sets="task.example">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <!--  In <example> when there is no <title> The "Example:" prefix does not appear  -->
...
-->
        ...
        <fo:block xsl:use-attribute-sets="note">
            ...
            <fo:inline xsl:use-attribute-sets="note__label">
                <xsl:choose>
                    ...
                    <!--      <xsl:when test="@type = 'laser_danger'">
                        <fo:inline xsl:use-attribute-sets="note__label__laser_danger">
                            <xsl:call-template name="insertVariable">
                                <xsl:with-param name="theVariableID" select="'Laser_Danger'"/>
                            </xsl:call-template>
                        </fo:inline>
                    </xsl:when>-->
    ...
    <xsl:template match="*[contains(@class, ' topic/draft-comment ')]">
        <xsl:if test="$publishRequiredCleanup = 'yes' or $DRAFT = 'yes'">
            <fo:block xsl:use-attribute-sets="draft-comment">
                <xsl:call-template name="commonattributes"/>
                <fo:block xsl:use-attribute-sets="draft-comment__label">Draft Comment: </fo:block>
    ...
    <xsl:template match="*" mode="determineTopicType">
        ...
        <xsl:choose>
            <!-- TOMV (2014-11-17) if topicSimple is inside of notices, we could give it a new topicType, such as "topicChildNotice" if it needs to be 
            handled with differnt font size than stnadard.. Not implementing for now.  
            -->
            <xsl:when test="ancestor-or-self::*[contains(@class, ' bookmap/notices ')]">
                <xsl:text>topicSimple</xsl:text>
                <!--  Dont use for now: topicChildNotice<xsl:text>topicChildNotice</xsl:text> -->
            </xsl:when>
            <xsl:otherwise>
                <xsl:text>topicSimple</xsl:text>
            </xsl:otherwise>

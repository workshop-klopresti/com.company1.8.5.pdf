...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                <fo:block xsl:use-attribute-sets="__frontmatter">
                        ...
                        <fo:block xsl:use-attribute-sets="__frontmatter__draft">
                            <xsl:choose>
                                <xsl:when test="$DRAFT = 'yes'">
                                    ...
                                    <fo:block>Draft Copy Not for Distribution</fo:block>
                                </xsl:when>
                                <xsl:otherwise/>
                            </xsl:choose>
                        </fo:block>
                    ...
                    <fo:block-container xsl:use-attribute-sets="__frontmatter__revision__container">
                        <fo:block xsl:use-attribute-sets="__frontmatter__revision">
                            Revision&#160;<xsl:value-of select="$documentRevision"/>
                        </fo:block>
                    </fo:block-container>
                </fo:block>
                <!--<xsl:call-template name="createPreface"/>-->
    ...
    <xsl:template match="*[contains(@class, ' bookmap/frontmatter ')]">
        <fo:block xsl:use-attribute-sets="__frontmatter_copyright">
            <xsl:apply-templates select="*[contains(@class, ' bookmap/frontmatter ')]"/>
        </fo:block>
    </xsl:template>
    ...
    <xsl:template match="*[contains(@class, ' bookmap/notices ')]" mode="process-notices">
        <xsl:variable name="topicType">
            <xsl:call-template name="determineTopicType"/>
        </xsl:variable>
    ...
    <!-- Add smaller font to copyright page  -->
